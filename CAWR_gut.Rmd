...
samp.comp<-ggplot(neword,aes(x=samps,y=value,fill=variable))+
  geom_bar(color="black",stat="identity")+coord_flip()+theme_minimal()+labs(x="Sample",y="Arthropod Composition \n(% of OTUs)")+theme(panel.background = element_rect(fill = "transparent",colour = NA))+scale_fill_brewer(name="Order",palette = 'Spectral')
                       
s<-ggplotly(samp.comp)
s2<-layout(s,xaxis=x,yaxis=y,autosize=F,width = 700, height = 450, margin = m)
s2
```
